...
class DataLoading:
    def __init__(self, macro_data: pd.DataFrame, bond_data: pd.DataFrame, market_data: pd.DataFrame, init_date):
        self._macro_data = macro_data.loc[macro_data.index >= init_date, ]
    ...
    def _arrange_data(self):
        ...
        self._macro_dict['pmi_ma12'] = util.ma(self._macro_data.iloc[:, 2], 12)
    ...
    def _set_a_share_index_erp(self):
        a_share_ep = np.divide(100, self._market_data.iloc[:, 1])
        erp = a_share_ep - self._macro_dict["rate_10yr"]
        return erp

    def _set_sp500_erp(self):
        sp_500_erp = np.divide(100, self._market_data.iloc[:, 4])
        erp = sp_500_erp - self._macro_dict["rate_10yr"]
        return erp

    def _set_sp500_vix(self):
        return self._market_data.iloc[:, 3]
